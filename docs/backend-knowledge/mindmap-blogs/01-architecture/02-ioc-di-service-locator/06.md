...
> 导图路径：构造器注入 → 可不可变  
> 节点类型：对象图装配节点；编号 6/13

## 1. 学习结论

“可不可变”是对象图装配节点。学完后必须能说明它的输入、状态所有权、执行顺序、不变量、失败传播和设计取舍。
...
源码阅读固定拆成五步：输入如何规范化；谁拥有可变状态；状态按什么顺序转移；输出/事件如何闭合；并发、异常和重试后什么条件仍必须成立。对“可不可变”而言，任何只覆盖正常返回、没有解释失败状态的实现都不完整。
...
从公开入口向下追踪到状态写入和外部适配器，再沿异常路径返回；记录“可不可变”在何处创建、验证、消费和释放。
...
下面是父专题中已经通过 Java 21 编译检查的机制实现。围绕“可不可变”定位相关字段、分支和保护条件：
...
1. 用 `rg "可不可变" src/main/java src/test/java` 搜索直接实现，再按本篇源码入口追踪间接实现；
...
**本质是什么？** 不只复述定义，要说清“可不可变”在 IoC、DI 与 Service Locator 中控制的状态或协议责任。
...
- **上一知识点**：[易测试：从概念到底层源码](05.md)
- **下一知识点**：[全局查询：从概念到底层源码](07.md)
