...
> 导图路径：更新 → 原子替换  
> 节点类型：并发控制节点；编号 12/14

## 1. 学习结论

“原子替换”是并发控制节点。学完后必须能说明它的输入、状态所有权、执行顺序、不变量、失败传播和设计取舍。
...
源码阅读固定拆成五步：输入如何规范化；谁拥有可变状态；状态按什么顺序转移；输出/事件如何闭合；并发、异常和重试后什么条件仍必须成立。对“原子替换”而言，任何只覆盖正常返回、没有解释失败状态的实现都不完整。
...
从公开入口向下追踪到状态写入和外部适配器，再沿异常路径返回；记录“原子替换”在何处创建、验证、消费和释放。
...
下面是父专题中已经通过 Java 21 编译检查的机制实现。围绕“原子替换”定位相关字段、分支和保护条件：
...
1. 用 `rg "原子替换" src/main/java src/test/java` 搜索直接实现，再按本篇源码入口追踪间接实现；
...
**本质是什么？** 不只复述定义，要说清“原子替换”在 配置系统与不可变快照 中控制的状态或协议责任。
...
- **上一知识点**：[最小暴露：从概念到底层源码](11.md)
- **下一知识点**：[重建客户端：从概念到底层源码](13.md)
