# Rule/Skill Cache：从概念到底层源码

> 所属专题：[对象作用域、全局状态与生命周期](../../../01-architecture/03-scope-lifecycle.md)  
> 导图路径：Workspace → Rule/Skill Cache  
> 节点类型：对象作用域与资源所有权节点；编号 5/12

## 1. 学习结论

“Rule/Skill Cache”是对象作用域与资源所有权节点。学完后必须能说明它的输入、状态所有权、执行顺序、不变量、失败传播和设计取舍。

## 2. 概念与项目语境

在“对象作用域、全局状态与生命周期”中，该节点位于“Workspace”分支。父专题中与它最接近的上下文如下：
...
源码阅读固定拆成五步：输入如何规范化；谁拥有可变状态；状态按什么顺序转移；输出/事件如何闭合；并发、异常和重试后什么条件仍必须成立。对“Rule/Skill Cache”而言，任何只覆盖正常返回、没有解释失败状态的实现都不完整。
...
从公开入口向下追踪到状态写入和外部适配器，再沿异常路径返回；记录“Rule/Skill Cache”在何处创建、验证、消费和释放。
...
下面是父专题中已经通过 Java 21 编译检查的机制实现。围绕“Rule/Skill Cache”定位相关字段、分支和保护条件：
...
1. 用 `rg "Rule/Skill Cache" src/main/java src/test/java` 搜索直接实现，再按本篇源码入口追踪间接实现；
...
**本质是什么？** 不只复述定义，要说清“Rule/Skill Cache”在 对象作用域、全局状态与生命周期 中控制的状态或协议责任。
...
- **上一知识点**：[WorkspaceContext：从概念到底层源码](04.md)
- **下一知识点**：[Conversation：从概念到底层源码](06.md)
