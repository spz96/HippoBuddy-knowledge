# SubAgentTask：从概念到底层源码

> 所属专题：[对象作用域、全局状态与生命周期](../../../01-architecture/03-scope-lifecycle.md)  
> 导图路径：Task → SubAgentTask  
> 节点类型：对象作用域与资源所有权节点；编号 11/12

## 1. 学习结论

“SubAgentTask”是对象作用域与资源所有权节点。学完后必须能说明它的输入、状态所有权、执行顺序、不变量、失败传播和设计取舍。

## 2. 概念与项目语境

在“对象作用域、全局状态与生命周期”中，该节点位于“Task”分支。父专题中与它最接近的上下文如下：
...
对象从创建、发布、使用到关闭必须由同一所有权链管理，依赖对象按正序创建、逆序释放。

源码阅读固定拆成五步：输入如何规范化；谁拥有可变状态；状态按什么顺序转移；输出/事件如何闭合；并发、异常和重试后什么条件仍必须成立。对“SubAgentTask”而言，任何只覆盖正常返回、没有解释失败状态的实现都不完整。
...
从公开入口向下追踪到状态写入和外部适配器，再沿异常路径返回；记录“SubAgentTask”在何处创建、验证、消费和释放。

## 5. 为什么要这样实现

这样设计防止全局状态污染、会话串扰和资源在 Map 删除后仍未真正关闭。

更简单的写法通常把规则散落到调用者，短期代码少，长期会导致入口行为不一致、半初始化/半提交状态、无法恢复或难以替换测试依赖。

## 6. 完整代码实现

下面是父专题中已经通过 Java 21 编译检查的机制实现。围绕“SubAgentTask”定位相关字段、分支和保护条件：
...
1. 用 `rg "SubAgentTask" src/main/java src/test/java` 搜索直接实现，再按本篇源码入口追踪间接实现；
...
**本质是什么？** 不只复述定义，要说清“SubAgentTask”在 对象作用域、全局状态与生命周期 中控制的状态或协议责任。
...
- **上一知识点**：[SseWriter：从概念到底层源码](10.md)
- **下一知识点**：[Bash Process：从概念到底层源码](12.md)
