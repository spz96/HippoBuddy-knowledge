# Conversation：从概念到底层源码

> 所属专题：[分层架构与应用服务](../../../01-architecture/01-layered-architecture.md)  
> 导图路径：领域层 → Conversation  
> 节点编号：7/15

## 1. 学习结论

它属于核心状态或规则层，负责维持跨多步操作仍然成立的业务约束。

学完本篇应能回答：它处理什么输入、改变什么状态、维持什么不变量、失败会怎样传播，以及为什么不能把这段责任随意放到其他层。

## 2. 概念与项目语境

在“分层架构与应用服务”中，**Conversation** 位于“领域层”分支。它不是孤立术语，而是整条后端调用链上的职责节点。
...
底层依靠聚合状态、显式方法和不可变快照把多字段变化收束到一个入口，而不是让 Handler 任意修改集合。
...
阅读时不要只看这一段代码，要从公开入口向下追踪到状态写入和外部适配器，再沿异常分支返回。针对“Conversation”，重点记录它在何处被创建、验证、消费和释放。

## 5. 为什么要这样实现

这样实现能把顺序、配对和预算等规则集中验证；若绕过该边界，单个字段虽然合法，整体状态仍可能不可恢复。

如果省略“Conversation”这一层控制，最常见后果是：协议细节侵入核心、状态顺序不可复现、失败后无法判断哪些副作用已经发生，或测试必须启动真实外部依赖。

## 6. 完整代码实现与阅读焦点

下面是父专题中通过 Java 21 编译检查的机制完整实现。阅读本节点时，重点查找与“Conversation”对应的输入、状态字段、分支与失败保护：
...
- **边界错放**：把“Conversation”的规则放到调用者，会出现多个入口行为不一致；
...
1. 在项目中用 `rg "Conversation" src/main/java src/test/java` 搜索直接命中；
...
**问：Conversation 的本质是什么？**  回答时不要只给定义，要指出它在 分层架构与应用服务 中拥有的状态或协议责任。
...
- **上一知识点**：[调用顺序：从概念到底层源码](06.md)
- **下一知识点**：[Rule/Skill：从概念到底层源码](08.md)
