...
在“分层架构与应用服务”中，**Rule/Skill** 位于“领域层”分支。它不是孤立术语，而是整条后端调用链上的职责节点。
...
阅读时不要只看这一段代码，要从公开入口向下追踪到状态写入和外部适配器，再沿异常分支返回。针对“Rule/Skill”，重点记录它在何处被创建、验证、消费和释放。

## 5. 为什么要这样实现

这样实现能把顺序、配对和预算等规则集中验证；若绕过该边界，单个字段虽然合法，整体状态仍可能不可恢复。

如果省略“Rule/Skill”这一层控制，最常见后果是：协议细节侵入核心、状态顺序不可复现、失败后无法判断哪些副作用已经发生，或测试必须启动真实外部依赖。

## 6. 完整代码实现与阅读焦点

下面是父专题中通过 Java 21 编译检查的机制完整实现。阅读本节点时，重点查找与“Rule/Skill”对应的输入、状态字段、分支与失败保护：
...
- **边界错放**：把“Rule/Skill”的规则放到调用者，会出现多个入口行为不一致；
...
1. 在项目中用 `rg "Rule/Skill" src/main/java src/test/java` 搜索直接命中；
...
**问：Rule/Skill 的本质是什么？**  回答时不要只给定义，要指出它在 分层架构与应用服务 中拥有的状态或协议责任。
...
- **上一知识点**：[Conversation：从概念到底层源码](07.md)
- **下一知识点**：[Truncation：从概念到底层源码](09.md)
