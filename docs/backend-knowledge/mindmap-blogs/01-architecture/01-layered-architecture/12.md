...
在“分层架构与应用服务”中，**MCP** 位于“基础设施”分支。它不是孤立术语，而是整条后端调用链上的职责节点。
...
阅读时不要只看这一段代码，要从公开入口向下追踪到状态写入和外部适配器，再沿异常分支返回。针对“MCP”，重点记录它在何处被创建、验证、消费和释放。

## 5. 为什么要这样实现

这样做隔离供应商与 I/O 变化，并允许测试用 Fake 替换真实网络或磁盘。

如果省略“MCP”这一层控制，最常见后果是：协议细节侵入核心、状态顺序不可复现、失败后无法判断哪些副作用已经发生，或测试必须启动真实外部依赖。

## 6. 完整代码实现与阅读焦点

下面是父专题中通过 Java 21 编译检查的机制完整实现。阅读本节点时，重点查找与“MCP”对应的输入、状态字段、分支与失败保护：
...
- **边界错放**：把“MCP”的规则放到调用者，会出现多个入口行为不一致；
...
1. 在项目中用 `rg "MCP" src/main/java src/test/java` 搜索直接命中；
...
**问：MCP 的本质是什么？**  回答时不要只给定义，要指出它在 分层架构与应用服务 中拥有的状态或协议责任。
...
- **上一知识点**：[文件系统：从概念到底层源码](11.md)
- **下一知识点**：[单向依赖：从概念到底层源码](13.md)
