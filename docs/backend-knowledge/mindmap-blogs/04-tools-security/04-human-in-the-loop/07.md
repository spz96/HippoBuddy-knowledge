# one-time consume：安全原理、源码与实现

> 所属专题：[Human-in-the-loop 与两阶段执行](../../../04-tools-security/04-human-in-the-loop.md)  
> 导图路径：Approval → one-time consume  
> 类型：人工确认授权节点；编号 7/14

## 1. 学习结论

“one-time consume”是人工确认授权节点。掌握标准是能说明信任边界、校验顺序、授权依据、副作用时机和失败后的状态。
...
围绕“one-time consume”沿模型可见定义追到真正 executor，确认任何绕过 Prompt 的直接调用仍会经过同一安全链。
...
围绕“one-time consume”修改参数、版本、路径或时序，验证拒绝发生在副作用之前。
...
1. 用 `rg "one-time consume" src/main/java src/test/java` 搜索并沿执行链补全；
...
- **上一知识点**：[expiry：安全原理、源码与实现](06.md)
- **下一知识点**：[revalidate：安全原理、源码与实现](08.md)
