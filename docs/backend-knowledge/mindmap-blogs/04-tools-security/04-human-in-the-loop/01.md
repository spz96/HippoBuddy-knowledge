...
> 导图路径：Prepare → validate  
> 类型：“Human-in-the-loop 与两阶段执行”中的安全执行节点；编号 1/14

## 1. 学习结论

“validate”是“Human-in-the-loop 与两阶段执行”中的安全执行节点。掌握标准是能说明信任边界、校验顺序、授权依据、副作用时机和失败后的状态。
...
围绕“validate”沿模型可见定义追到真正 executor，确认任何绕过 Prompt 的直接调用仍会经过同一安全链。

## 5. 为什么这样实现

这样让 Prompt 之外仍有不可绕过的执行边界。
...
围绕“validate”修改参数、版本、路径或时序，验证拒绝发生在副作用之前。
...
1. 用 `rg "validate" src/main/java src/test/java` 搜索并沿执行链补全；
...
- **上一知识点**：[case folding：安全原理、源码与实现](../03-path-sandbox/12.md)
- **下一知识点**：[preview impact：安全原理、源码与实现](02.md)
