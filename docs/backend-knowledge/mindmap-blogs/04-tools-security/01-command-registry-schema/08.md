# JSON Schema：安全原理、源码与实现

> 所属专题：[Command、Tool Registry 与 JSON Schema](../../../04-tools-security/01-command-registry-schema.md)  
> 导图路径：Validation → JSON Schema  
> 类型：Tool Registry 与契约节点；编号 8/13

## 1. 学习结论

“JSON Schema”是Tool Registry 与契约节点。掌握标准是能说明信任边界、校验顺序、授权依据、副作用时机和失败后的状态。
...
围绕“JSON Schema”沿模型可见定义追到真正 executor，确认任何绕过 Prompt 的直接调用仍会经过同一安全链。
...
围绕“JSON Schema”修改参数、版本、路径或时序，验证拒绝发生在副作用之前。
...
1. 用 `rg "JSON Schema" src/main/java src/test/java` 搜索并沿执行链补全；
...
- **上一知识点**：[JSON syntax：安全原理、源码与实现](07.md)
- **下一知识点**：[business rule：安全原理、源码与实现](09.md)
