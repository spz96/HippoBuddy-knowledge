# head and tail：安全原理、源码与实现

> 所属专题：[工具输出分类与截断策略](../../../04-tools-security/06-output-truncation.md)  
> 导图路径：Preserve → head and tail  
> 类型：结构感知截断节点；编号 11/14

## 1. 学习结论

“head and tail”是结构感知截断节点。掌握标准是能说明信任边界、校验顺序、授权依据、副作用时机和失败后的状态。
...
围绕“head and tail”沿模型可见定义追到真正 executor，确认任何绕过 Prompt 的直接调用仍会经过同一安全链。
...
围绕“head and tail”修改参数、版本、路径或时序，验证拒绝发生在副作用之前。
...
1. 用 `rg "head and tail" src/main/java src/test/java` 搜索并沿执行链补全；
...
- **上一知识点**：[hierarchy：安全原理、源码与实现](10.md)
- **下一知识点**：[per-tool limit：安全原理、源码与实现](12.md)
