...
“list”是结构感知截断节点。掌握标准是能说明信任边界、校验顺序、授权依据、副作用时机和失败后的状态。
...
围绕“list”沿模型可见定义追到真正 executor，确认任何绕过 Prompt 的直接调用仍会经过同一安全链。
...
围绕“list”修改参数、版本、路径或时序，验证拒绝发生在副作用之前。
...
1. 用 `rg "list" src/main/java src/test/java` 搜索并沿执行链补全；
...
- **上一知识点**：[diff：安全原理、源码与实现](03.md)
- **下一知识点**：[tree：安全原理、源码与实现](05.md)
