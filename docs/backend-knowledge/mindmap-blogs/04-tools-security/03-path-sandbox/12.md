# case folding：安全原理、源码与实现

> 所属专题：[路径规范化、Sandbox 与符号链接](../../../04-tools-security/03-path-sandbox.md)  
> 导图路径：Attacks → case folding  
> 类型：路径解析与 Sandbox 节点；编号 12/12

## 1. 学习结论

“case folding”是路径解析与 Sandbox 节点。掌握标准是能说明信任边界、校验顺序、授权依据、副作用时机和失败后的状态。
...
围绕“case folding”沿模型可见定义追到真正 executor，确认任何绕过 Prompt 的直接调用仍会经过同一安全链。
...
围绕“case folding”修改参数、版本、路径或时序，验证拒绝发生在副作用之前。
...
1. 用 `rg "case folding" src/main/java src/test/java` 搜索并沿执行链补全；
...
- **上一知识点**：[TOCTOU：安全原理、源码与实现](11.md)
- **下一知识点**：[validate：安全原理、源码与实现](../04-human-in-the-loop/01.md)
