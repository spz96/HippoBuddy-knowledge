# normalize：安全原理、源码与实现

> 所属专题：[路径规范化、Sandbox 与符号链接](../../../04-tools-security/03-path-sandbox.md)  
> 导图路径：Canonicalization → normalize  
> 类型：路径解析与 Sandbox 节点；编号 3/12

## 1. 学习结论

“normalize”是路径解析与 Sandbox 节点。掌握标准是能说明信任边界、校验顺序、授权依据、副作用时机和失败后的状态。
...
围绕“normalize”沿模型可见定义追到真正 executor，确认任何绕过 Prompt 的直接调用仍会经过同一安全链。
...
围绕“normalize”修改参数、版本、路径或时序，验证拒绝发生在副作用之前。
...
1. 用 `rg "normalize" src/main/java src/test/java` 搜索并沿执行链补全；
...
- **上一知识点**：[absolute：安全原理、源码与实现](02.md)
- **下一知识点**：[real path：安全原理、源码与实现](04.md)
