# reverse diff：安全原理、源码与实现

> 所属专题：[乐观并发、快照与补偿事务](../../../04-tools-security/05-edit-snapshot-compensation.md)  
> 导图路径：Compensation → reverse diff  
> 类型：乐观并发与补偿节点；编号 8/12

## 1. 学习结论

“reverse diff”是乐观并发与补偿节点。掌握标准是能说明信任边界、校验顺序、授权依据、副作用时机和失败后的状态。
...
围绕“reverse diff”沿模型可见定义追到真正 executor，确认任何绕过 Prompt 的直接调用仍会经过同一安全链。
...
围绕“reverse diff”修改参数、版本、路径或时序，验证拒绝发生在副作用之前。
...
1. 用 `rg "reverse diff" src/main/java src/test/java` 搜索并沿执行链补全；
...
- **上一知识点**：[undo：安全原理、源码与实现](07.md)
- **下一知识点**：[version check：安全原理、源码与实现](09.md)
