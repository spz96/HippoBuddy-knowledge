...
> 导图路径：Compensation → undo  
> 类型：乐观并发与补偿节点；编号 7/12

## 1. 学习结论

“undo”是乐观并发与补偿节点。掌握标准是能说明信任边界、校验顺序、授权依据、副作用时机和失败后的状态。
...
围绕“undo”沿模型可见定义追到真正 executor，确认任何绕过 Prompt 的直接调用仍会经过同一安全链。
...
围绕“undo”修改参数、版本、路径或时序，验证拒绝发生在副作用之前。
...
1. 用 `rg "undo" src/main/java src/test/java` 搜索并沿执行链补全；
...
- **上一知识点**：[metadata：安全原理、源码与实现](06.md)
- **下一知识点**：[reverse diff：安全原理、源码与实现](08.md)
