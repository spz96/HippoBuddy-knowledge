# reject count：底层原理、源码与实现

> 所属专题：[有界线程池、队列与背压](../../../02-concurrency-network/02-bounded-pool-backpressure.md)  
> 导图路径：指标 → reject count  
> 类型：“有界线程池、队列与背压”中的并发控制节点；编号 11/11

## 1. 学习结论

“reject count”是“有界线程池、队列与背压”中的并发控制节点。掌握标准是能从线程/状态/协议层说明正常路径、竞争路径、超时路径和关闭路径。
...
- [ForkAgentsTool.java](../../../../../src/main/java/com/example/agent/tools/ForkAgentsTool.java)
...
围绕“reject count”追踪创建、进入临界区/协议帧、状态提交、异常转换和 finally 释放位置。
...
把与“reject count”有关的保护删除或移位，再用 latch/barrier、短 deadline 或强制异常稳定复现失败，不要只用 sleep 猜测时序。
...
1. 用 `rg "reject count" src/main/java src/test/java` 定位直接实现并沿调用链补全间接实现；
...
- **上一知识点**：[wait time：底层原理、源码与实现](10.md)
- **下一知识点**：[消息有序：底层原理、源码与实现](../03-session-lock-atomicity/01.md)
