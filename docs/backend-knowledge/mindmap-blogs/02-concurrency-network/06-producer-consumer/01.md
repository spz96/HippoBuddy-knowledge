...
“有界”是容量与背压节点。掌握标准是能从线程/状态/协议层说明正常路径、竞争路径、超时路径和关闭路径。
...
围绕“有界”追踪创建、进入临界区/协议帧、状态提交、异常转换和 finally 释放位置。
...
把与“有界”有关的保护删除或移位，再用 latch/barrier、短 deadline 或强制异常稳定复现失败，不要只用 sleep 猜测时序。
...
1. 用 `rg "有界" src/main/java src/test/java` 定位直接实现并沿调用链补全间接实现；
...
- **上一知识点**：[backpressure：底层原理、源码与实现](../05-sse/12.md)
- **下一知识点**：[FIFO：底层原理、源码与实现](02.md)
