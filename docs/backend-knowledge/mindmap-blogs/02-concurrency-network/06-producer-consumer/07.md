...
> 导图路径：背压 → 阻塞  
> 类型：“生产者—消费者、批处理与背压”中的并发控制节点；编号 7/13

## 1. 学习结论

“阻塞”是“生产者—消费者、批处理与背压”中的并发控制节点。掌握标准是能从线程/状态/协议层说明正常路径、竞争路径、超时路径和关闭路径。
...
围绕“阻塞”追踪创建、进入临界区/协议帧、状态提交、异常转换和 finally 释放位置。
...
把与“阻塞”有关的保护删除或移位，再用 latch/barrier、短 deadline 或强制异常稳定复现失败，不要只用 sleep 猜测时序。
...
1. 用 `rg "阻塞" src/main/java src/test/java` 定位直接实现并沿调用链补全间接实现；
...
- **上一知识点**：[flush：底层原理、源码与实现](06.md)
- **下一知识点**：[拒绝：底层原理、源码与实现](08.md)
