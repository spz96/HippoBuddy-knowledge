# final answer：底层原理、源码与实现

> 所属专题：[Agent Loop 与状态机](../../../03-agent-llm/01-agent-loop-state-machine.md)  
> 导图路径：Policy → final answer  
> 类型：Agent 状态机节点；编号 6/14

## 1. 学习结论

“final answer”是Agent 状态机节点。掌握它不是背 API，而是能解释协议字段、运行时状态、可信边界、终止条件和失败恢复。
...
围绕“final answer”追踪请求构造、状态写入、Provider/Tool 边界、异常转换和终态发布。
...
定位与“final answer”相关的状态字段和 invariant；删除校验、拆分 arguments 或制造部分成功，观察失败如何出现。
...
1. 用 `rg "final answer" src/main/java src/test/java` 定位直接或相关实现；
...
- **上一知识点**：[LLM decides action：底层原理、源码与实现](05.md)
- **下一知识点**：[validates：底层原理、源码与实现](07.md)
