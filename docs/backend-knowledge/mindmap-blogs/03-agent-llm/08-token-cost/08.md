# tool result：底层原理、源码与实现

> 所属专题：[Token Usage、价格与成本治理](../../../03-agent-llm/08-token-cost.md)  
> 导图路径：来源 → tool result  
> 类型：Prompt Cache 身份节点；编号 8/14

## 1. 学习结论

“tool result”是Prompt Cache 身份节点。掌握它不是背 API，而是能解释协议字段、运行时状态、可信边界、终止条件和失败恢复。
...
围绕“tool result”追踪请求构造、状态写入、Provider/Tool 边界、异常转换和终态发布。
...
定位与“tool result”相关的状态字段和 invariant；删除校验、拆分 arguments 或制造部分成功，观察失败如何出现。
...
1. 用 `rg "tool result" src/main/java src/test/java` 定位直接或相关实现；
...
- **下一知识点**：[subagent：底层原理、源码与实现](09.md)
