...
> 导图路径：来源 → history  
> 类型：Token 与成本治理节点；编号 6/14

## 1. 学习结论

“history”是Token 与成本治理节点。掌握它不是背 API，而是能解释协议字段、运行时状态、可信边界、终止条件和失败恢复。
...
围绕“history”追踪请求构造、状态写入、Provider/Tool 边界、异常转换和终态发布。
...
定位与“history”相关的状态字段和 invariant；删除校验、拆分 arguments 或制造部分成功，观察失败如何出现。
...
1. 用 `rg "history" src/main/java src/test/java` 定位直接或相关实现；
