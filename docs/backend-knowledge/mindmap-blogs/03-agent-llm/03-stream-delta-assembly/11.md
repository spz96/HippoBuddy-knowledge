# finish reason：底层原理、源码与实现

> 所属专题：[SSE Delta 解析与增量合并](../../../03-agent-llm/03-stream-delta-assembly.md)  
> 导图路径：完成 → finish reason  
> 类型：流式增量组装节点；编号 11/13

## 1. 学习结论

“finish reason”是流式增量组装节点。掌握它不是背 API，而是能解释协议字段、运行时状态、可信边界、终止条件和失败恢复。
...
围绕“finish reason”追踪请求构造、状态写入、Provider/Tool 边界、异常转换和终态发布。
...
定位与“finish reason”相关的状态字段和 invariant；删除校验、拆分 arguments 或制造部分成功，观察失败如何出现。
...
1. 用 `rg "finish reason" src/main/java src/test/java` 定位直接或相关实现；
...
- **上一知识点**：[arguments buffer：底层原理、源码与实现](10.md)
- **下一知识点**：[validate size：底层原理、源码与实现](12.md)
