# malformed SSE：底层原理、源码与实现

> 所属专题：[错误分类与异常语义](../../../03-agent-llm/06-error-taxonomy.md)  
> 导图路径：Protocol → malformed SSE  
> 类型：流式增量组装节点；编号 5/16

## 1. 学习结论

“malformed SSE”是流式增量组装节点。掌握它不是背 API，而是能解释协议字段、运行时状态、可信边界、终止条件和失败恢复。
...
围绕“malformed SSE”追踪请求构造、状态写入、Provider/Tool 边界、异常转换和终态发布。
...
定位与“malformed SSE”相关的状态字段和 invariant；删除校验、拆分 arguments 或制造部分成功，观察失败如何出现。
...
1. 用 `rg "malformed SSE" src/main/java src/test/java` 定位直接或相关实现；
...
- **上一知识点**：[timeout：底层原理、源码与实现](04.md)
- **下一知识点**：[invalid JSON：底层原理、源码与实现](06.md)
