# system message：底层原理、源码与实现

> 所属专题：[多供应商 Adapter 与统一模型](../../../03-agent-llm/04-provider-adapter.md)  
> 导图路径：Provider 差异 → system message  
> 类型：Provider 适配节点；编号 6/12

## 1. 学习结论

“system message”是Provider 适配节点。掌握它不是背 API，而是能解释协议字段、运行时状态、可信边界、终止条件和失败恢复。
...
围绕“system message”追踪请求构造、状态写入、Provider/Tool 边界、异常转换和终态发布。
...
定位与“system message”相关的状态字段和 invariant；删除校验、拆分 arguments 或制造部分成功，观察失败如何出现。
...
1. 用 `rg "system message" src/main/java src/test/java` 定位直接或相关实现；
...
- **上一知识点**：[auth headers：底层原理、源码与实现](05.md)
- **下一知识点**：[tool schema：底层原理、源码与实现](07.md)
