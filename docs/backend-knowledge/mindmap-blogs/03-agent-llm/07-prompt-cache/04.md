# system prompt：底层原理、源码与实现

> 所属专题：[Prompt 前缀缓存](../../../03-agent-llm/07-prompt-cache.md)  
> 导图路径：稳定内容 → system prompt  
> 类型：Provider 适配节点；编号 4/13

## 1. 学习结论

“system prompt”是Provider 适配节点。掌握它不是背 API，而是能解释协议字段、运行时状态、可信边界、终止条件和失败恢复。
...
围绕“system prompt”追踪请求构造、状态写入、Provider/Tool 边界、异常转换和终态发布。
...
定位与“system prompt”相关的状态字段和 invariant；删除校验、拆分 arguments 或制造部分成功，观察失败如何出现。
...
1. 用 `rg "system prompt" src/main/java src/test/java` 定位直接或相关实现；
...
- **上一知识点**：[provider policy：底层原理、源码与实现](03.md)
- **下一知识点**：[tool schemas：底层原理、源码与实现](05.md)
