# JSON Schema：底层原理、源码与实现

> 所属专题：[Function Calling 与工具协议](../../../03-agent-llm/02-function-calling.md)  
> 导图路径：Contract → JSON Schema  
> 类型：Function Calling 协议节点；编号 3/14

## 1. 学习结论

“JSON Schema”是Function Calling 协议节点。掌握它不是背 API，而是能解释协议字段、运行时状态、可信边界、终止条件和失败恢复。
...
围绕“JSON Schema”追踪请求构造、状态写入、Provider/Tool 边界、异常转换和终态发布。
...
定位与“JSON Schema”相关的状态字段和 invariant；删除校验、拆分 arguments 或制造部分成功，观察失败如何出现。
...
1. 用 `rg "JSON Schema" src/main/java src/test/java` 定位直接或相关实现；
...
- **上一知识点**：[description：底层原理、源码与实现](02.md)
- **下一知识点**：[call id：底层原理、源码与实现](04.md)
