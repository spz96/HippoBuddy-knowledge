...
> 导图路径：Runtime → validate  
> 类型：Function Calling 协议节点；编号 7/14

## 1. 学习结论

“validate”是Function Calling 协议节点。掌握它不是背 API，而是能解释协议字段、运行时状态、可信边界、终止条件和失败恢复。
...
围绕“validate”追踪请求构造、状态写入、Provider/Tool 边界、异常转换和终态发布。
...
定位与“validate”相关的状态字段和 invariant；删除校验、拆分 arguments 或制造部分成功，观察失败如何出现。
...
1. 用 `rg "validate" src/main/java src/test/java` 定位直接或相关实现；
...
- **上一知识点**：[arguments：底层原理、源码与实现](06.md)
- **下一知识点**：[authorize：底层原理、源码与实现](08.md)
