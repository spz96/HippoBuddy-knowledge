...
“permanent”是重试与幂等节点。掌握它不是背 API，而是能解释协议字段、运行时状态、可信边界、终止条件和失败恢复。
...
围绕“permanent”追踪请求构造、状态写入、Provider/Tool 边界、异常转换和终态发布。
...
定位与“permanent”相关的状态字段和 invariant；删除校验、拆分 arguments 或制造部分成功，观察失败如何出现。
...
1. 用 `rg "permanent" src/main/java src/test/java` 定位直接或相关实现；
...
- **上一知识点**：[transient：底层原理、源码与实现](01.md)
- **下一知识点**：[ambiguous：底层原理、源码与实现](03.md)
