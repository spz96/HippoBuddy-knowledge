# exponential backoff：底层原理、源码与实现

> 所属专题：[重试、指数退避与幂等](../../../03-agent-llm/05-retry-backoff-idempotency.md)  
> 导图路径：时间策略 → exponential backoff  
> 类型：重试与幂等节点；编号 4/10

## 1. 学习结论

“exponential backoff”是重试与幂等节点。掌握它不是背 API，而是能解释协议字段、运行时状态、可信边界、终止条件和失败恢复。
...
围绕“exponential backoff”追踪请求构造、状态写入、Provider/Tool 边界、异常转换和终态发布。
...
定位与“exponential backoff”相关的状态字段和 invariant；删除校验、拆分 arguments 或制造部分成功，观察失败如何出现。
...
1. 用 `rg "exponential backoff" src/main/java src/test/java` 定位直接或相关实现；
...
- **上一知识点**：[ambiguous：底层原理、源码与实现](03.md)
- **下一知识点**：[max delay：底层原理、源码与实现](05.md)
