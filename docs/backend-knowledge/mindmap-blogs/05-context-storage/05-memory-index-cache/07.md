# persistent context：底层原理、源码与实现

> 所属专题：[Markdown 记忆、索引与缓存失效](../../../05-context-storage/05-memory-index-cache.md)  
> 导图路径：Cache → persistent context  
> 类型：记忆真相、索引与缓存节点；编号 7/12

## 1. 学习结论

“persistent context”是记忆真相、索引与缓存节点。掌握标准是能区分真相状态、派生索引/缓存、持久化确认点与失败恢复点。
...
- [MemoryToolSandbox.java](../../../../../src/main/java/com/example/agent/memory/MemoryToolSandbox.java)
...
围绕“persistent context”追踪数据从内存状态到日志/文件/索引，再从持久化记录恢复为运行时对象的双向路径。
...
围绕“persistent context”制造越级阈值、partial write、重复事件、外部编辑、索引损坏或检索 miss，验证恢复策略。
...
1. 用 `rg "persistent context" src/main/java src/test/java` 定位相关代码；
...
- **上一知识点**：[startup scan：底层原理、源码与实现](06.md)
- **下一知识点**：[version key：底层原理、源码与实现](08.md)
