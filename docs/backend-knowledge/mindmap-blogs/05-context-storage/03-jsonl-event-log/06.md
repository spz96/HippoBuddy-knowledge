# tail corruption isolation：底层原理、源码与实现

> 所属专题：[JSONL、Append-only Log 与 Event Sourcing](../../../05-context-storage/03-jsonl-event-log.md)  
> 导图路径：Append Only → tail corruption isolation  
> 类型：JSONL 日志与重放节点；编号 6/12

## 1. 学习结论

“tail corruption isolation”是JSONL 日志与重放节点。掌握标准是能区分真相状态、派生索引/缓存、持久化确认点与失败恢复点。
...
围绕“tail corruption isolation”追踪数据从内存状态到日志/文件/索引，再从持久化记录恢复为运行时对象的双向路径。
...
围绕“tail corruption isolation”制造越级阈值、partial write、重复事件、外部编辑、索引损坏或检索 miss，验证恢复策略。
...
1. 用 `rg "tail corruption isolation" src/main/java src/test/java` 定位相关代码；
...
- **上一知识点**：[immutable history：底层原理、源码与实现](05.md)
- **下一知识点**：[conversation state：底层原理、源码与实现](07.md)
