...
围绕“migration”追踪数据从内存状态到日志/文件/索引，再从持久化记录恢复为运行时对象的双向路径。
...
围绕“migration”制造越级阈值、partial write、重复事件、外部编辑、索引损坏或检索 miss，验证恢复策略。
...
1. 用 `rg "migration" src/main/java src/test/java` 定位相关代码；
...
- **上一知识点**：[compaction：底层原理、源码与实现](11.md)
- **下一知识点**：[queue：底层原理、源码与实现](../04-file-durability/01.md)
