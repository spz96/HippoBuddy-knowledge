# operations：底层原理、源码与实现

> 所属专题：[文件存储、SQLite 与 PostgreSQL 选型](../../../05-context-storage/07-storage-selection.md)  
> 导图路径：PostgreSQL → operations  
> 类型：存储选型节点；编号 10/12

## 1. 学习结论

“operations”是存储选型节点。掌握标准是能区分真相状态、派生索引/缓存、持久化确认点与失败恢复点。
...
围绕“operations”追踪数据从内存状态到日志/文件/索引，再从持久化记录恢复为运行时对象的双向路径。
...
围绕“operations”制造越级阈值、partial write、重复事件、外部编辑、索引损坏或检索 miss，验证恢复策略。
...
1. 用 `rg "operations" src/main/java src/test/java` 定位相关代码；
...
- **上一知识点**：[transactions：底层原理、源码与实现](09.md)
- **下一知识点**：[large outputs：底层原理、源码与实现](11.md)
