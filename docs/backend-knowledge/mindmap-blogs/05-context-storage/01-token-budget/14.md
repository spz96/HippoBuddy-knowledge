...
> 导图路径：Actions → block  
> 类型：Token 编码与预算节点；编号 14/14

## 1. 学习结论

“block”是Token 编码与预算节点。掌握标准是能区分真相状态、派生索引/缓存、持久化确认点与失败恢复点。
...
围绕“block”追踪数据从内存状态到日志/文件/索引，再从持久化记录恢复为运行时对象的双向路径。
...
围绕“block”制造越级阈值、partial write、重复事件、外部编辑、索引损坏或检索 miss，验证恢复策略。
...
1. 用 `rg "block" src/main/java src/test/java` 定位相关代码；
...
- **上一知识点**：[compact：底层原理、源码与实现](13.md)
- **下一知识点**：[deterministic：底层原理、源码与实现](../02-context-compaction/01.md)
