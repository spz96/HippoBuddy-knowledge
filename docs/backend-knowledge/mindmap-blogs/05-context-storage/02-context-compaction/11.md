# boundary id：底层原理、源码与实现

> 所属专题：[滑动窗口、摘要压缩与 Session Memory](../../../05-context-storage/02-context-compaction.md)  
> 导图路径：State → boundary id  
> 类型：上下文压缩节点；编号 11/13

## 1. 学习结论

“boundary id”是上下文压缩节点。掌握标准是能区分真相状态、派生索引/缓存、持久化确认点与失败恢复点。
...
围绕“boundary id”追踪数据从内存状态到日志/文件/索引，再从持久化记录恢复为运行时对象的双向路径。
...
围绕“boundary id”制造越级阈值、partial write、重复事件、外部编辑、索引损坏或检索 miss，验证恢复策略。
...
1. 用 `rg "boundary id" src/main/java src/test/java` 定位相关代码；
...
- **上一知识点**：[pending work：底层原理、源码与实现](10.md)
- **下一知识点**：[compaction count：底层原理、源码与实现](12.md)
