...
> 导图路径：Prompt → identity  
> 类型：Prompt/Rule/Skill/Tool 信任节点；编号 1/11

## 1. 学习结论

“identity”是Prompt/Rule/Skill/Tool 信任节点。掌握标准是能说明协议/状态、信任来源、生命周期、验证方法和失败恢复。
...
围绕“identity”从注册/初始化追踪到执行、事件、清理和故障路径，区分项目当前能力与未来设计。
...
围绕“identity”注入依赖失败、超时、重连、恶意输入或并发终态，检查代码是否保持唯一且可观测的结果。
...
1. 用 `rg "identity" src/main/java src/test/java` 追踪源码；
2. 画初始化、执行、故障和关闭时序；
...
- **上一知识点**：[reconnect：底层原理、源码与实现](../02-jsonrpc-mcp/14.md)
- **下一知识点**：[behavior：底层原理、源码与实现](02.md)
