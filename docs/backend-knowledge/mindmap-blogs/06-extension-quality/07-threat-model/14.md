# data exfiltration：底层原理、源码与实现

> 所属专题：[Agent 安全威胁模型](../../../06-extension-quality/07-threat-model.md)  
> 导图路径：Threats → data exfiltration  
> 类型：Agent 威胁模型节点；编号 14/22

## 1. 学习结论

“data exfiltration”是Agent 威胁模型节点。掌握标准是能说明协议/状态、信任来源、生命周期、验证方法和失败恢复。
...
资产和信任边界先建模，外部文本始终标为不可信；每个副作用入口以最小 capability、sandbox、确认、出站策略和审计重新授权。
...
围绕“data exfiltration”从注册/初始化追踪到执行、事件、清理和故障路径，区分项目当前能力与未来设计。

## 5. 为什么这样实现

这样阻断从 Prompt Injection 到工具执行/数据外泄的跨层攻击链。
...
围绕“data exfiltration”注入依赖失败、超时、重连、恶意输入或并发终态，检查代码是否保持唯一且可观测的结果。
...
1. 用 `rg "data exfiltration" src/main/java src/test/java` 追踪源码；
2. 画初始化、执行、故障和关闭时序；
...
- **上一知识点**：[command execution：底层原理、源码与实现](13.md)
- **下一知识点**：[approval replay：底层原理、源码与实现](15.md)
