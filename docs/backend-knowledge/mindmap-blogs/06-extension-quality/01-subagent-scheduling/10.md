...
> 导图路径：State → running  
> 类型：子 Agent 状态与调度节点；编号 10/15

## 1. 学习结论

“running”是子 Agent 状态与调度节点。掌握标准是能说明协议/状态、信任来源、生命周期、验证方法和失败恢复。
...
围绕“running”从注册/初始化追踪到执行、事件、清理和故障路径，区分项目当前能力与未来设计。
...
围绕“running”注入依赖失败、超时、重连、恶意输入或并发终态，检查代码是否保持唯一且可观测的结果。
...
1. 用 `rg "running" src/main/java src/test/java` 追踪源码；
2. 画初始化、执行、故障和关闭时序；
...
- **上一知识点**：[waiting：底层原理、源码与实现](09.md)
- **下一知识点**：[terminal：底层原理、源码与实现](11.md)
