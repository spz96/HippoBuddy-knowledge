/**
 * PptxSilurusPreview — @silurus/ooxml PPTX 预览
 *
 * 使用 Silurus 引擎的 PptxScrollViewer 渲染 PPTX 预览。
...
/**
 * 使用 @silurus/ooxml 渲染 PPTX 预览
 * @param {HTMLElement} container - 渲染目标容器
 * @param {string} filePath - 文件路径
 * @param {boolean} [forceRefresh] - 是否强制刷新
 * @param {Function} [onError] - 错误回调 (err) => void
 */
export async function renderPptxSilurus(container, filePath, forceRefresh, onError) {
  const encodedPath = encodeURIComponent(filePath);
  const cacheBust = forceRefresh ? `&_t=${Date.now()}` : '';
  const url = `/api/file/raw?path=${encodedPath}${cacheBust}`;

  try {
    container.innerHTML = `<div class="file-binary-preview loading">加载 PPTX 文件中（Silurus 引擎）...</div>`;
